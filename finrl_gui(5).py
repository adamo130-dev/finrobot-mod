...
class TradingApp:
    ...
    def setup_screener_section(self):
        ...
        ttk.Label(self.screener_frame, text="P/E Ratio:").pack(side="left", padx=5)
        self.pe_ratio = ttk.Entry(self.screener_frame, width=15)
        self.pe_ratio.pack(side="left", padx=5)

        ttk.Label(self.screener_frame, text="Min. Volume:").pack(side="left", padx=5)
        self.min_volume = ttk.Entry(self.screener_frame, width=15)
        self.min_volume.pack(side="left", padx=5)

        ttk.Label(self.screener_frame, text="RSI Min:").pack(side="left", padx=5)
        self.rsi_min = ttk.Entry(self.screener_frame, width=7)
        self.rsi_min.pack(side="left", padx=5)
        self.rsi_min.insert(0, "30")

        ttk.Label(self.screener_frame, text="RSI Max:").pack(side="left", padx=5)
        self.rsi_max = ttk.Entry(self.screener_frame, width=7)
        self.rsi_max.pack(side="left", padx=5)
    ...
    def run_screener(self):
        ...
        # Get S&P 500 stocks as our universe
        sp500 = yf.download("^GSPC")
        tickers = yf.Tickers("^GSPC").tickers['^GSPC'].info['components']
        screened_stocks = []
        ...
        for symbol in tickers:
            try:
                ...
                if len(stock_data) < 50:  # Skip if not enough data
                    continue

                # Calculate technical indicators
                sma20 = calculate_sma(stock_data, 20)
                sma50 = calculate_sma(stock_data, 50)
                sma200 = calculate_sma(stock_data, 200)
                bb_mid, bb_upper, bb_lower = calculate_bollinger_bands(stock_data)

                # Get fundamentals
                try:
                    pe_ratio = float(info.get('trailingPE', float('inf')))
                    market_cap = float(info.get('marketCap', 0)) / 1e6  # Convert to millions
                ...
                if self.market_cap.get():
                    if market_cap < float(self.market_cap.get()):
                        meets_criteria = False
                if self.pe_ratio.get():
                    if pe_ratio > float(self.pe_ratio.get()):
                        meets_criteria = False
                ...
                # Placeholder for TA_Handler to avoid errors. Will be properly configured later if needed
                analysis = TA_Handler(
                        symbol="AAPL",
                        screener="america",
                        exchange="NASDAQ",
                ...
                if float(self.rsi_min.get()) <= rsi <= float(self.rsi_max.get()):
                    meets_criteria = meets_criteria and True
                else:
                    meets_criteria = False

                # Additional indicators
                if self.sma_var.get():
                    meets_criteria = meets_criteria and sma20.iloc[-1] > sma50.iloc[-1]

                if self.macd_var.get():
                    meets_criteria = meets_criteria and analysis.indicators["MACD.macd"] > analysis.indicators["MACD.signal"]

                if self.bb_var.get():
                    current_price = stock_data['Close'].iloc[-1]
                    meets_criteria = meets_criteria and bb_lower.iloc[-1] < current_price < bb_upper.iloc[-1]
        ...
        self.results_text.insert("end", "Screened Stocks:\n\n")
        self.results_text.insert("end", f"{'Symbol':<10} {'Market Cap(M)':<15} {'P/E':<10} {'Volume(M)':<12} {'RSI':<8}\n")
        self.results_text.insert("end", "-" * 55 + "\n")
        ...
        for stock in screened_stocks:
            self.results_text.insert("end", 
                f"{stock['symbol']:<10} "
                f"{stock['market_cap']:<15,.2f} "
                f"{stock['pe_ratio']:<10,.2f} "
                f"{stock['volume']:<12,.2f} "
                f"{stock['rsi']:<8,.2f}\n"
            )
